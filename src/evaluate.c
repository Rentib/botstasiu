...
static Score material_score[6]; /* [PieceType] */
static Score table[2][6][64];   /* [Color][PieceType][Square] */

void
initialise_evaluation(void)
{
  for (PieceType pt = PAWN; pt <= KING; pt++) {
    material_score[pt] = S(mg_value[pt], eg_value[pt]);
    for (Square sq = SQ_A8; sq <= SQ_H1; sq++) {
      for (Color side = WHITE; side <= BLACK; side++, sq = FLIP(sq)) {
        table[side][pt][sq] = S(mg_pesto_table[pt][sq], eg_pesto_table[pt][sq]);
      }
    }
  }
}
...
evaluate(Position *pos)
{
  ...
  for (side = WHITE; side <= BLACK; side++) {
    for (pt = PAWN; pt <= KING; pt++) {
      mask = pos->color[side] & pos->piece[pt];
      int cnt = popcount(mask);
      gamePhase += cnt * gamephaseInc[pt];
      score[side].mg += cnt * material_score[pt].mg;
      score[side].eg += cnt * material_score[pt].eg;
      while (mask) {
        sq = pop_lsb(&mask);
        score[side].mg += table[side][pt][sq].mg;
        score[side].eg += table[side][pt][sq].eg;
      }
    }
  }

  /* tapered eval */
  Score final = ScoreSub(score[pos->turn], score[!pos->turn]);
  int mgPhase = gamePhase;
  if (mgPhase > 24) mgPhase = 24; /* in case of early promotion */
  int egPhase = 24 - mgPhase;
  return (final.mg * mgPhase + final.eg * egPhase) / 24;
}
